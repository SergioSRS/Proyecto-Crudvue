...
export function VistaModificar(controlador){
	return Vue.createApp({
		...
		`
		<div :class=clase>
			<h2 class="tituloProceso">Modificacion de juegos</h2>
				<div class="formulario">
					<label>id<input v-model="id" style="display:none" /> </label>
				...
				<button @click="modificar">Aceptar</button>
				</div>
		</div>`,
		methods: {
			mostrar(activo){
			...
			modificar(){
				...
				this.controlador.modificar(this.id,this.nombre,this.precio,this.fecha,this.descripcion,this.edad,this.subirImagen[0])
